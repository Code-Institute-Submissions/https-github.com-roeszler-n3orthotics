...
* When a user searched for an order that was not not matched in the database, the local 'previously input' data was being keep in the local list. This was throwing an error when the user then tried to re-enter the correct information. Fixed by adding a .clear() method after a failed match. 
