...
* When a user searched for an order that was not not matched in the database, the local 'previously input' row data was staying in the search data. This was throwing a TypeError when the user then tried to re-enter the correct information. Solved by containing the output integer from the `retrieve_order()` to a defined list `search_row[0]` and defining diferently in the `display_order()` function.
